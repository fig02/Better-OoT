...
class Message:
    ...
    def is_id_message(self):
        if self.unpadded_length == 5:
            for i in range(4):
                ...
                if not (code in range(ord('0'), ord('9') + 1) or code in range(ord('A'), ord('F') + 1) or code in range(
                        ord('a'), ord('f') + 1)):
                    return False
            return True
        return False
    ...
    def is_basic(self):
        return not (
                    self.has_goto or self.has_keep_open or self.has_event or self.has_fade or self.has_ocarina or self.has_two_choice or self.has_three_choice)
    ...
    def write(self, rom, index, offset, replace_ending=False, ending=None, always_allow_skip=True, speed_up_text=True):
        ...
        # # speed the text only if ids dont = specificed text boxes
        # ! OPTING OUT OF SPECIFIC QUICK TEXT HAPPENS HERE

        if speed_up_text:

            if self.id in not_quick_text_ids:
                pass
            else:
...
def repack_messages(rom, messages, permutation=None, always_allow_skip=True, speed_up_text=True):
    ...
    if offset > TEXT_SIZE_LIMIT:
        raise (TypeError(
            "Message Text table is too large: 0x" + "{:x}".format(offset) + " written / 0x" + "{:x}".format(
                TEXT_SIZE_LIMIT) + " allowed."))
    ...
    if 8 * (table_index + 1) > TABLE_SIZE_LIMIT:
        raise (TypeError("Message ID table is too large: 0x" + "{:x}".format(
            8 * (table_index + 1)) + " written / 0x" + "{:x}".format(TABLE_SIZE_LIMIT) + " allowed."))
...
def shuffle_messages(rom, except_hints=True, always_allow_skip=True):
    ...
    def is_not_exempt(m):
        ...
        exempt_as_hint = (except_hints and m.id in (
                    GOSSIP_STONE_MESSAGES + TEMPLE_HINTS_MESSAGES + LIGHT_ARROW_HINT + list(KEYSANITY_MESSAGES.keys())))
